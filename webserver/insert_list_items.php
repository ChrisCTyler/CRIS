   ...
   //Decode JSON into an Array
    ...
    if (mysqli_connect_errno($con))
    {
        ...
        if (! mysqli_begin_transaction($con, MYSQLI_TRANS_START_READ_WRITE)) {
            ...
            $stmt = mysqli_prepare($con, "REPLACE ListItem (RecordID, ListItemID, HistoryDate, SyncID, CreationDate, CreatedByID, ListType, ItemValue, IsDisplayed, SerialisedObject) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ? ,?)");
            foreach($data as $label=>$value){
                switch ($label) {
                    ...
                    default:
                        ...
                        mysqli_stmt_bind_param($stmt, "ssisisssis", $value["RecordID"],$value["ListItemID"],$value["HistoryDate"],$value["SyncID"],$value["CreationDate"],$value["CreatedByID"],$value["ListType"],$value["ItemValue"],$value["IsDisplayed"],base64_decode($value["SerialisedObject"]));
                        if (!mysqli_stmt_execute($stmt)) {
                            $output['result'] = "FAILURE";
                            $output['error_message'] = "PHP:ListItem: (" . $label . ") - " . mysqli_stmt_error($stmt);
                        }
                }
            }
