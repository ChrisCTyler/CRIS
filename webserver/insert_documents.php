   ...
   //Decode JSON into an Array
    ...
    if (mysqli_connect_errno($con))
    {
        ...
        if (! mysqli_begin_transaction($con, MYSQLI_TRANS_START_READ_WRITE)) {
            ...
            $stmt = mysqli_prepare($con, "REPLACE Document (RecordID, DocumentID, HistoryDate, SyncID, CreationDate, CreatedByID, Cancelled, ClientID, DocumentType, SerialisedObject) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ? ,?)");
            foreach($data as $label=>$value){
                switch ($label) {
                    ...
                    default:
                        ...
                        mysqli_stmt_bind_param($stmt, "ssisisisss", $value["RecordID"],$value["DocumentID"],$value["HistoryDate"],$value["SyncID"],$value["CreationDate"],$value["CreatedByID"],$value["Cancelled"],$value["ClientID"],$value["DocumentType"],base64_decode($value["SerialisedObject"]));
                        if (!mysqli_stmt_execute($stmt)) {
                            $output['result'] = "FAILURE";
                            $output['error_message'] = "PHP:Document: (" . $label . ") - " . mysqli_stmt_error($stmt);
                        }
                }
            }
