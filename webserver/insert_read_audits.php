   ...
   //Decode JSON into an Array
    ...
    if (mysqli_connect_errno($con))
    {
        ...
        if (! mysqli_begin_transaction($con, MYSQLI_TRANS_START_READ_WRITE)) {
            ...
            $stmt = mysqli_prepare($con, "REPLACE ReadAudit (SyncID, ReadDate, ReadByID, DocumentID) VALUES (?, ?, ?, ?)");
            foreach($data as $label=>$value){
                switch ($label) {
                    ...
                    default:
                        ...
                        mysqli_stmt_bind_param($stmt, "siss", $value["SyncID"],$value["ReadDate"],$value["ReadByID"],$value["DocumentID"]);
                        if (!mysqli_stmt_execute($stmt)) {
                            $output['result'] = "FAILURE";
                            $output['error_message'] = "PHP:ReadAudit: (" . $label . ") - " . mysqli_stmt_error($stmt);
                        }
                }
            }
