...
// Build 107 - Rewritten to use PDO rather than mysqli due to website hosting problems
 ...
 $file = 'pdo_insert_documents_18.php';
...
try {
    ...
    $JSONData = $_POST['JSONData'];
     ...
     $sql = "REPLACE Document (RecordID, DocumentID, HistoryDate, SyncID, CreationDate, CreatedByID, Cancelled, ClientID, DocumentType, SerialisedObject, ReferenceDate, SessionID) ";
     $sql .= "VALUES (:recordid, :documentid, :historydate, :syncid, :creationdate, :createbyid, :cancelled, :clientid, :documenttype, :serialisedobject, :referencedate, :sessionid)";
     ...
     foreach($data as $label=>$value){
          switch ($label) {
              ...
              default:
                  ...
                  $sth->bindValue(':createbyid', $value["CreatedByID"], PDO::PARAM_STR);
                  $sth->bindValue(':cancelled', $value["Cancelled"], PDO::PARAM_INT);
                  $sth->bindValue(':clientid', $value["ClientID"], PDO::PARAM_STR);
                  $sth->bindValue(':documenttype', $value["DocumentType"], PDO::PARAM_STR);
                  $sth->bindValue(':serialisedobject', base64_decode($value["SerialisedObject"]), PDO::PARAM_LOB);
                  $sth->bindValue(':referencedate', $value["ReferenceDate"], PDO::PARAM_INT);
                  $sth->bindValue(':sessionid', $value["SessionID"], PDO::PARAM_STR);
                  if (!$sth->execute()){
                      throw new Exception($sth->errorInfo()[2]);
                  }
          }
