   ...
   //Decode JSON into an Array
    ...
    if (mysqli_connect_errno($con))
    {
        ...
        if (! mysqli_begin_transaction($con, MYSQLI_TRANS_START_READ_WRITE)) {
            ...
            $stmt = mysqli_prepare($con, "REPLACE SystemError (RecordID, SyncID, CreationDate, CreatedByID, SerialisedObject) VALUES (?, ?, ?, ? ,?)");
            foreach($data as $label=>$value){
                switch ($label) {
                    ...
                    default:
                        ...
                        mysqli_stmt_bind_param($stmt, "ssiss", $value["RecordID"],$value["SyncID"],$value["CreationDate"],$value["CreatedByID"],base64_decode($value["SerialisedObject"]));
                        if (!mysqli_stmt_execute($stmt)) {
                            $output['result'] = "FAILURE";
                            $output['error_message'] = "PHP:SystemError: (" . $label . ") - " . mysqli_stmt_error($stmt);
                        }
                }
            }
